...
class Scheduler(object):
    ...
    '''def get_scheduling(self): #busca reservas no banco
        ...
        self.__list.clear()
        logs.log("INFO - Buscando reservas...")
        query = 'https://teste.agendamento.rnp.br/rnp-agendamento/api/reunioes/todas?dataInicial='+today+'&dataFinal='+tomorrow+'&sala={}'.format(self.__id) #monta a query
    ...
    def consumer(self): #publica horarios de reserva
        ...
        if len(self.__list) > 0: #se há reservas
            for row in self.__list:
                #print(row["end"] <= (datetime.now() + timedelta(seconds=5)))#and row["end"] <= (datetime.now() + timedelta(seconds=5)))
                if row["start"] >= datetime.now() and row["start"] <= (datetime.now() + timedelta(seconds=60)): #verifica se a reserva começa em menos de um minuto
                    #print(datetime.now() + timedelta(seconds=5))
                    msg = 'i|{}|f|{}'.format(str(row["start"]),str(row["end"]))
                    logs.log('INFO - Publicando: {}'.format(json.dumps(msg))) #publica horario
                    try:
                        response = self.__client.publish(self.__config['topics']['topic_scheduler'], msg)
                        time.sleep(2)
                        response.wait_for_publish()
                        self.__list.remove(row) #remove da lista
