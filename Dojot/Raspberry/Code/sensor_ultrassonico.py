...
class Ultrassonico(object):
    def __init__(self):
        ...
        self.log("INFO - Subscribing to topic {}...".format(self.__config['topics']['subscribe']['topic_scheduler']))
            ...
            if dist > 10: # verifica se a distancia é maior que 10 (sem presença) e se o monitoramento nao foi encerrado
                ...
                while self.distance() > 10 and not self.__end and datetime.now() <= time_stop - timedelta(minutes=5): # verifica se a distancia é maior que 10 (sem presença)
                    if datetime.now() >= start + timedelta(seconds=30) and not self.__end: # verifica se já se passaram 30 seg sem presença  e se o monitoramento nao foi encerrado
                        ...
                        self.__client.publish(self.__config['topics']['publish']['topic_raspberry'], json.dumps(msg))
                        time.sleep(1)
                        start = datetime.now() #inicia o temporizador de 30 seg
    ...
    def subscribe_schedule(self):
        subscribe.callback(self.on_message_schedule, self.__config['topics']['subscribe']['topic_scheduler'], hostname= self.__config['mqtt_broker']['host'])

    def heartbeat(self):
        msg = {'atividade': self.__id, 'time_msg': str(datetime.now())}
        self.log("INFO - Nó ativo: {}".format(msg))
        response = self.__client.publish(self.__config['topics']['publish']['topic_heartbeat'], json.dumps(msg))
        time.sleep(1)
